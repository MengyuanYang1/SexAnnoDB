...
inpath = '/data2/myang9/SexAnnoDB/output/Network/TF_Gene/WebFiguresTF/files/'
outpath = '/data2/myang9/SexAnnoDB/output/Network/TF_Gene/WebFiguresTF/figures/'

inf = str_c(inpath,f,sep='/')
outf = str_c(outpath,geneid,'_TF-Gene_forTF.svg',sep='')
...
my_Plot <- function(d_graph){
  ggraph(d_graph, layout = "stress") +
    ...
    geom_node_point(size=5,alpha = 0.3,color='black',fill='black')+
...
svg(outf,ceiling(n_can/n_row)*3,n_row*3)
