...
plot(tmp2,  main=paste0('Top20 female biased RBP-ES regulation in ',cancer_type),
     layout= layout_with_kk,#  layout.kamada.kawai,
     vertex.label.cex = .75,
     edge.arrow.size=1,
     edge.label.cex = 0.75,
...
net1 <- net1[,1:3]
net2 <- net2[,1:3]
names(net1) <- c('RBP','ES_events','Score')
write.table(net1,paste0('/data2/myang9/SexAnnoDB/output/Network/RBP_ES/',cancer_type,'_RBP_ES_Summary_Maletop20.txt'),sep="\t",row.names=FALSE,quote=FALSE)
write.table(net2,paste0('/data2/myang9/SexAnnoDB/output/Network/RBP_ES/',cancer_type,'_RBP_ES_Summary_Femaletop20.txt'),sep="\t",row.names=FALSE,quote=FALSE)
