...
sign_m = args[4]
cancer_type =  args[5]
#f = 'ENSG00000153495-rs1536053_ENSG00000153495-ACC-sQTL.txt'
#svgf = 'ENSG00000153495-rs1536053_ENSG00000153495-ACC-sQTL.svg'
#sign_f='***'
#sign_m='ns'
size=10
print(f)
...
#d$y <- log10(d$y)
d$y <- as.numeric(d$y)
FPKM_max = max(d$y)*0.9
p1 <- ggplot(d, aes(x=x, y=y,color=Group)) +geom_boxplot() + facet_grid(.~Group)
anno <- data.frame(xstar = c(2, 2), ystar = c(FPKM_max, FPKM_max),lab = c(sign_f, sign_m),Group = c("Female", "Male"))
p1 <- p1 +labs(x='',title = cancer_type,y="FPKM")
#p1 <- p1 + xlim(0,1.05)
